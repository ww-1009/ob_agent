...
│   │   │   ├── tools.py      # 10 DBA tools + 2 read-only doc file tools (12 registered)
...
2. **`backend/doc/`**: OceanBase official documentation knowledge base. The repository ships the compressed archive `backend/doc/ob_wiki.zip`; unzip it in place before starting the backend, which yields `backend/doc/ob_wiki/` (the extracted files are gitignored). The System Prompt expects the documentation entry point at `./doc/ob_wiki/README.md`; because the file tools are rooted at `./doc`, the agent reads it as `ob_wiki/README.md` in read-only mode. **Missing this directory will cause document retrieval features to fail**.
