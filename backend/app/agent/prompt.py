...
def system_prompt(now: datetime | None = None) -> str:
    ...
    return (
        ...
        "and never answer from a navigation index page (`index.md`) or the knowledge-base guide "
        "(`README.md`) instead of the body text: they are down-weighted by default so the body wins. "
        "Set `include_index=true` only when the user asks for a list or taxonomy (\"有哪些 / 包含哪些 "
        "/ 怎么分类\"), where the category index pages are the right source; even then only the listed "
        "items come from them, and every explanation must come from the body text.\n"
        "Rules:\n"
