...
- ``mysql``：PyMySQL 直连 MySQL 模式租户 ``MysqlSqlExecutor``
- ``oracle``：OCI 驱动（oracledb / cx_Oracle 可切换）连 Oracle 模式租户 ``OracleSqlExecutor``
- ``registry``：真实执行器长连接注册表，供应用关闭时统一释放

执行器与模块都按方言命名（``mock`` / ``mysql`` / ``oracle``，类名 ``MockSqlExecutor`` /
``MysqlSqlExecutor`` / ``OracleSqlExecutor`` 同构）；旧名 ``RealSqlExecutor`` /
``OBOracleSqlExecutor`` 与旧模块名 ``sql.real`` 已随之重命名。
